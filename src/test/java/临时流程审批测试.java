...
//                continue;
    ...
    public void 康帅或者刘杰送审() {
        ...
        approveVO.setResourceId("20200826154320");
        approveVO.setType(1);
        ...
        if (判断是否有该业务健对应的处在激活状态的流程实例("临时合同审批流程",approveVO.getResourceId())) {
            throw new RuntimeException();
        }


        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("临时合同审批流程", approveVO.getResourceId(), vars);
...
//        String[] run = new String[]{"liujie:17","hangyezongjian:9","suixin:7","jtliu:6"};
        ...
        for (String s : run) {
            ...
            //实际业务中判断流程实例是否已经完成了
            Task task = taskService.createTaskQuery().processDefinitionKey("临时合同审批流程").processInstanceBusinessKey("20200826154320").active().singleResult();
    ...
    public void 查询某个合同的审批链路历史() {
        String resourceId = "20200826154320";
