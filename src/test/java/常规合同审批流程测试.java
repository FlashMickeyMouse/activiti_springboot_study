...
public class 常规合同审批流程测试 {
    ...
    @Test
    public void test() {
        Authentication.setAuthenticatedUserId("songhao_start");
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("测试获取执行人", "HT001");
    ...
    public void getcurrenttask() {
        ...
        List<Task> list = taskService.createTaskQuery().taskCandidateGroup("currentUser_role").or().taskAssignee("currentUser_id").active().list();

        List<Task> taskList = taskService.createTaskQuery().processInstanceBusinessKey("HT001").active().list();
    ...
    public void salesStart() {
        ...
        approveVO.setRemark("");
        approveVO.setResourceId("20200313");
        approveVO.setType(1);


        //判断为合同类型，并且为常规合同，所以走常规合同审批流程 动作为送审
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("常规合同审批流程", approveVO.getResourceId());
        ...
        List<IdentityLink> identityLinksForTask = taskService.getIdentityLinksForTask(task.getId());
        IdentityLink identityLink = identityLinksForTask.get(0);
        System.out.println(identityLink.getUserId());
        System.out.println(identityLink.getGroupId());
        if ("dynamic.current.dep.leader".equals(identityLink.getUserId())) {
            //假装 业务根据规则dynamic.current.dep.leader上查询了下当前人的部门领导为刘杰 角色name为综合行业华北区域总监
            //设置该任务的名称和代办人
            taskService.addCandidateGroup(task.getId(), "17");
            //因为17这个角色是单人 所以直接设置如果是多人则直接设置为null等待下一个角色查看代办之后认领 由于activiti sql查询语句的关系必须设置为null 才能通过角色查出来
//            task.setAssignee("liujie");
//            task.setName("综合行业华北区域总监审批");
            //taskService.setAssignee(task.getId(), "liujie");
            taskService.setVariableLocal(task.getId(), "taskShowName", "综合行业华北区域总监");
        }
        System.out.println(taskService.getVariablesLocal(task.getId()).get("taskShowName"));
...
//        上一级部门领导.put("17", new Role("9", "综合行业总监"));
    ...
    public void 康帅或者刘杰送审() {
        ...
        approveVO.setResourceId("20200826154319");
        approveVO.setType(1);
        System.err.println(approveVO.getResourceId());
        String startUserId = "kangshuai";
        //判断为合同类型，并且为常规合同，所以走常规合同审批流程 动作为送审
        //设置流程变量
        ...
        if (判断是否有该业务健对应的处在激活状态的流程实例(approveVO.getResourceId())) {
            throw new RuntimeException();
        }


        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("常规合同审批流程", approveVO.getResourceId(), vars);
...
//        String[] run = new String[]{"liujie:17","hangyezongjian:9","zhangxuejie:5","fangyan:4","zhangqing:3","suixin:7","jtliu:6"};
        String[] run = new String[]{"liujie:17","zhangxuejie:5","fangyan:4","zhangqing:3","suixin:7","jtliu:6"};
        for (String s : run) {
            String[] strs = s.split(":");
            String currentUserName = strs[0];
            String currentUserId = strs[1];
            //实际业务中判断流程实例是否已经完成了
            Task task = taskService.createTaskQuery().processInstanceBusinessKey("20200826154319").active().singleResult();
    ...
    public void 查询某个合同的审批链路历史() {
        String resourceId = "20200826154319";
...
//                Object taskShowName = taskService.getVariableLocal(taskId, "taskShowName");
    ...
    private boolean 判断是否有该业务健对应的处在激活状态的流程实例(String businessKey) {
        List<ProcessInstance> list = runtimeService.createProcessInstanceQuery().processInstanceBusinessKey(businessKey).active().list();
